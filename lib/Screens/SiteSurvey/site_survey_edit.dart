...
import 'package:flutterapp/Responses/GetSurveyDetailsResponse.dart';
...
class SiteSurveyEditPage extends StatefulWidget {
  ...
  @override
  _SiteSurveyEditPageState createState() => _SiteSurveyEditPageState(
...
class _SiteSurveyEditPageState extends State<SiteSurveyEditPage> {
  final String surveyId;
  final String projectId;
  final String title;
  final String gpName;
  ...
  int counter = 0;
  List<File> tempList = new List();

  Future<List<File>> urlToFile(List<SurveyImage> surveyImagesList) async {
    print('urlList.length: '+ surveyImagesList.length.toString());

    try{

          surveyImagesList.forEach((element) async {
            if(tempList.length<surveyImagesList.length) {
              Directory tempDir = await getTemporaryDirectory();
              String tempPath = tempDir.path;
              File file = new File(
                  '$tempPath' + (element.id).toString() + '.png');

              http.Response response = await http.get(element.image);

              print('response: ' + element.id.toString() +
...
//            ImageList.add(file);
              ...
              print('ImageList length: ' + tempList.toString());
            }
          });
      ...
      print('exceptioniurl: '+e.toString());
    }
...
//    }
            ...
            child: Column(
              children: <Widget>[
                Container(
                  ...
                  child: FutureBuilder(
                    future: getSurveyDetailsList(widget.surveyId),
                    builder: (context, res){

                      Future<List<File>> survey_images_future;
                      ...
                      if(!res.hasData){
                        ...
                        GetSurveyDetailsResponse getSurveyDetailsResponse = GetSurveyDetailsResponse.fromJson(res.data);

                        if(counter==0){

                        _selectedGpfeasibility = getSurveyDetailsResponse.message.details.feasibility==null? 'No':
                        getSurveyDetailsResponse.message.details.feasibility.toString()=='0'? 'No': 'Yes';
                        print('getSurveyDetailsResponse.message.details.feasibility: '+_selectedGpfeasibility);

                        _selectedReasonlist = getSurveyDetailsResponse.message.details.reason==null? 'AP not installed':
                        getSurveyDetailsResponse.message.details.reason.toString();

                        _selectedOntAvailableList = getSurveyDetailsResponse.message.details.ontAvailable==null? 'No':
                        getSurveyDetailsResponse.message.details.ontAvailable.toString()=='0'? 'No': 'Yes';

                        _selectedOntPowerStateList = getSurveyDetailsResponse.message.details.ontPowerStatus==null? 'No':
                        getSurveyDetailsResponse.message.details.ontPowerStatus.toString()=='0'? 'No': 'Yes';

                        _selectedMakeModelList = getSurveyDetailsResponse.message.details.ontMakeModel==null? 'No':
                        getSurveyDetailsResponse.message.details.ontMakeModel.toString()=='0'? 'No': 'Yes';

                        _selectedPoeStatusList = getSurveyDetailsResponse.message.details.poeStatus==null? 'No':
                        getSurveyDetailsResponse.message.details.poeStatus.toString()=='0'? 'No': 'Yes';

                        _selectedAlarmStatusList = getSurveyDetailsResponse.message.details.alarmStatus==null? 'No':
                        getSurveyDetailsResponse.message.details.alarmStatus.toString()=='0'? 'No': 'Yes';

                        _selectedSolarPanelAvailableList = getSurveyDetailsResponse.message.details.solarPanelAvailable==null? 'No':
                        getSurveyDetailsResponse.message.details.solarPanelAvailable.toString()=='0'? 'No': 'Yes';

                        _selectedCcuAvailableList = getSurveyDetailsResponse.message.details.ccuAvailable==null? 'No':
                        getSurveyDetailsResponse.message.details.ccuAvailable.toString()=='0'? 'No': 'Yes';

                        _selectedBatteryAvailableList = getSurveyDetailsResponse.message.details==null? 'No':
                        getSurveyDetailsResponse.message.details.reason.toString()=='0'? 'No': 'Yes';

                        _selectedPowerRunning = getSurveyDetailsResponse.message.details.powerRunning==null? 'No':
                        getSurveyDetailsResponse.message.details.powerRunning.toString()=='0'? 'No': 'Yes';


                        _cable_req_length.text = getSurveyDetailsResponse.message.details.cableReqLength==null? '0': getSurveyDetailsResponse.message.details.cableReqLength;
                        _macIdController.text = getSurveyDetailsResponse.message.details.ontMacId==null? '0': getSurveyDetailsResponse.message.details.ontMacId;


                        survey_images_future = urlToFile(getSurveyDetailsResponse.message.details.surveyImages);
                        counter++;

                        }

                        return FutureBuilder(
                          future: survey_images_future,
                          builder: (context, res1){
  ...
  Future<dynamic> getSurveyDetailsList(String survey_id) async {
    setState(() {
      isLoading = true;
    });
    isToken = false;
    var response = await http.get(
        'https://fieldapp.m2mcybernetics.com/api/v1/getSurveyDetails?token=$apiToken&survey_id=$survey_id',
        headers: {'authkey': 'Bcju0SQW2RKR0vhLzGbjeQwqkJvBUyA4IkMDuNubQq5'});
    if (response.statusCode == 200) {
      try {
        var responseJson = json.decode(response.body);
        _stopAnimation();
        print('getSurveryDetailsResponse: '+responseJson.toString());
        return responseJson;
      } catch (e) {
        _stopAnimation();
        print(e);
